...
def get_country_cert_files(country, certtype):
    ...
    if country == 'firstCountry' or country is None: 
        return (path.join(certificateFolder, "{certtype}.pem"), path.join(certificateFolder, "key_{certtype}.pem") )

    return (path.join(certificateFolder, country, "{certtype}.pem"), path.join(certificateFolder, country,"key_{certtype}.pem") )
